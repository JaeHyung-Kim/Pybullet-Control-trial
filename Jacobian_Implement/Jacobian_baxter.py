...
def accurateIK(bodyId, endEffectorId, targetPosition, lowerLimits, upperLimits, jointRanges, restPoses, 
    ...
    jointAngle = [0 for i in range(numJoints)]
    jointVel = [0 for i in range(numJoints)]
    jointAcc = [0 for i in range(numJoints)]
    for i in range(numJoints-1):
        js = p.getJointState(bodyId,i)
        jointAngle[i] = js[0] # save the current joint position
        jointVel[i] = js[1]
        jointAcc[i] = 1
    ...
    while (not closeEnough and iter<maxIter):
        ...
        jacobian = p.calculateJacobian(bodyId, endEffectorId, ls[4], jointAngle[endEffectorId-19:endEffectorId], jointVel[endEffectorId-19:endEffectorId], jointAcc[endEffectorId-19:endEffectorId])[0]
        inverse = np.linalg.pinv(jacobian)
        theta = np.matmul(inverse, diff)
